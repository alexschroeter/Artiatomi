...
Volume<tVoxel>::Volume(dim3 aDim)
  : _countSubVolumes(0), _data(NULL), _positionVolume(make_float3(0)), _positionSubVolume(NULL), _dimension(),
...
template<typename tVoxel>
Volume<tVoxel>::Volume(dim3 aDim, int aSubVolCount, int aSubVol)
  : _countSubVolumes(aSubVolCount), _data(NULL), _positionVolume(make_float3(0)), _positionSubVolume(NULL), _dimension(),
    _dimensionSubVolume(NULL), _voxelSize(make_float3(1)), _bitResolution(sizeof(tVoxel))
{
  _dimension.x = aDim.x;
  _dimension.y = aDim.y;
...
{
  // tVoxel max = -999999999; //SG!! there was a bug here for unsigned tVoxel types 
  tVoxel max = 0; //SG!! set initial value to 0
  ...
  for (int i = 0; i < _countSubVolumes; i++)
	...
	for (size_t v = 0; v < (size_t)_dimensionSubVolume[i].x * (size_t)_dimensionSubVolume[i].y * (size_t)_dimensionSubVolume[i].z; v++)
	  {
	    //max = std::max(max, voxels[v]); //SG!!! the line was commented out.
	  }
    }
  return max;
}
...
void Volume<tVoxel>::RemoveSubVolume(int x, int y, int z, int radius, tVoxel value, int part)
{
  ...
  if (zEndInPartVol > (int) _dimensionSubVolume[part].z)
    {
      zEndInPartVol = (int) _dimensionSubVolume[part].z;
    }
  ...
  if (yEnd > (int) _dimensionSubVolume[part].y)
    {
      yEnd = (int) _dimensionSubVolume[part].y;
    }
  ...
  if (xEnd > (int) _dimensionSubVolume[part].x)
    {
      xEnd = (int) _dimensionSubVolume[part].x;
    }
  ...
  for (int z = zStartInPartVol; z < zEndInPartVol; z++)
    {
      for (int y = yStart; y < yEnd; y++)
	{
	  for (int x = xStart; x < xEnd; x++)
	    {
	      int idx = z * _dimensionSubVolume[part].x * _dimensionSubVolume[part].y
