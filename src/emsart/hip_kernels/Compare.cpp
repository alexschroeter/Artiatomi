...
void compare( DevParamCompare recParam )
{
  
  int &proj_x = recParam.proj_x;
  int &proj_y = recParam.proj_y;
  size_t &stride = recParam.stride;
  float* &real_raw = recParam.real_raw; 
  float* &virtual_raw = recParam.virtual_raw; 
  float* &vol_distance_map = recParam.vol_distance_map; 
  float &realLength = recParam.realLength; 
  float4 &cutLength = recParam.cutLength; 
  float4 &dimLength = recParam.dimLength; 
  float &projValScale = recParam.projValScale;
  ...
  if (y < cutLength.z + dimLength.z)
    {
      ...
      if (w<0) w = 0;
      distYB = 1.0f - 1.0f/expf( w * w * 9.0f );
    }
  else 
    {
      distYB = 1.0f;
  ...
  if (y > proj_y - dimLength.w-cutLength.w - 1)
    {
      ...
      if (w<0) w = 0.0f;
      distYA = 1.0f - 1.0f/expf( w * w * 9.0f );
    }
  else
    {
      distYA = 1.0f;
  ...
  if (x < cutLength.y + dimLength.y)
    {
      ...
      if (w<0) w = 0;
      distXB = 1.0f - 1.0f/expf( w * w * 9.0f );
    }
  else 
    {
      distXB = 1.0f;
  ...
  if (x > proj_x - dimLength.x-cutLength.x - 1)
    {
      ...
      if (w<0) w = 0.0f;
      distXA = 1.0f - 1.0f/expf( w * w * 9.0f );
    }
  else
    {
      distXA = 1.0f;
...
__device__ float GetDistanceMy(int2 v, int2 w, int x, int y)
{
  //SG: more stable calculations
  ...
  if ( dot <= 0.0f ) return sqrt(x1*x1+y1*y1);
		
  if ( dot > l2 ) return distance(x, y, w.x, w.y);
...
void cropBorder( DevParamCropBorder recParam )
{
  int &proj_x = recParam.proj_x; 
  int &proj_y = recParam.proj_y; 
  size_t &stride = recParam.stride; 
  float* &image = recParam.image; 
  float2 &cutLength = recParam.cutLength; 
  float2 &dimLength = recParam.dimLength; 
  int2 &p1 = recParam.p1; 
  int2 &p2 = recParam.p2; 
  int2 &p3 = recParam.p3; 
  int2 &p4 = recParam.p4;
  ...
  if ( (p2.x - p1.x)*(y - p1.y) - (p2.y - p1.y)*(x - p1.x) < 0
       ...
       && (p4.x - p2.x)*(y - p2.y) - (p4.y - p2.y)*(x - p2.x) < 0)
    {
      ...
      minDistX = fminf(minDistX, GetDistanceMy(p1, p2, x, y));
      minDistX = fminf(minDistX, GetDistanceMy(p4, p3, x, y));
      minDistX = fminf(minDistX, x);
      minDistX = fminf(minDistX, proj_x - x - 1);
		
      minDistY = fminf(minDistY, GetDistanceMy(p3, p1, x, y));
      minDistY = fminf(minDistY, GetDistanceMy(p2, p4, x, y));
