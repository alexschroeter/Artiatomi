...
void textwrite(string aFileName, float *data, int width, int height) 
{
    ofstream f(aFileName);
    if (!f.is_open()) return;
    f << std::scientific << std::setw(20);
    ...
    for (int i = 0; i < height; i++) 
    {
        ...
        if (n)
            sum /= n;
        f << std::setw(3) << i << " ";
        f << std::setw(10) << std::setprecision(4) << sum;
        /*
        f<<" : ";
        int cnt=0;
        for( int j=0; (j<width) && (cnt<10); j++ ){
            if( data[i*width+j]==0.f ) continue;
            f<< std::setw(3) <<j<<": "<< std::setw(10) <<data[i*width+j]<<" ";
            cnt++;
        }
        */
        f << endl;
...
int main(int argc, char *argv[]) 
{
    ...
    try 
    {
        ...
        if (mpi_part == 0) 
        {
            ...
            printf("Volume shifts: %f, %f, %f\n", aConfig.VolumeShift.x,
                    aConfig.VolumeShift.y, aConfig.VolumeShift.z);
            printf("Volume file name: %s\n", aConfig.OutVolumeFile.c_str());
            printf("Lambda: %f\n", aConfig.Lambda);
            printf("Iterations: %i\n\n", aConfig.Iterations);
            fflush(stdout);
        }
        ...
        log << "Performing CTF correction: "
            << (aConfig.CtfMode != Configuration::Config::CTFM_NO ? "TRUE" : "FALSE") << endl;
        if (aConfig.CtfMode != Configuration::Config::CTFM_NO) 
        {
            log << "Ignore volume Z-shift for CTF correction: "
                << (aConfig.IgnoreZShiftForCTF ? "TRUE" : "FALSE") << endl;
            log << "Slice thickness for CTF correction in nm: "
                << aConfig.CTFSliceThickness << endl;
        }
        ...
        if (aConfig.RecDimensions.x % 4 != 0) 
        {
            ...
            log << "RecDimensions.x (" << aConfig.RecDimensions.x
                << ") is not a multiple of 4" << endl;
        }
        ...
        printf("Create HIP context on device %i ... \n", aConfig.HipDeviceIDs[mpi_offset + mpi_host_rank]);
        hipCtx = Hip::HipContext::CreateInstance(aConfig.HipDeviceIDs[mpi_offset + mpi_host_rank]);
        printf("Using HIP device %s\n", hipCtx->GetDeviceProperties()->GetDeviceName());
        printf("Available Memory on device: %lu MB\n", hipCtx->GetFreeMemorySize() / 1024 / 1024);
        fflush(stdout);
        ...
        if (mpi_part == 0) 
        {
            ...
            {
                printf("Error: Projection file format not supported. Supported formats "
                    "are: DM4 file series, MRC stacks, ST stacks.");
                log << SimpleLogger::LOG_ERROR;
                log << "Projection file format not supported. Supported formats are: "
                    "DM4 file series, MRC stacks, ST stacks."
                    << endl;
                WaitForInput(-1);
            }
        ...
        KernelModuls modules(hipCtx);
        float3 volSize;
        hipChannelFormatDesc arrayFormat;
        if (aConfig.FP16Volume) 
        {
            volSize = volFP16->GetSubVolumeDimension(mpi_part);
            arrayFormat = myhipCreateChannelDesc(16, 0, 0, 0, hipChannelFormatKindFloat);
        } 
        else 
        {
            volSize = vol->GetSubVolumeDimension(mpi_part);
            arrayFormat = myhipCreateChannelDesc(32, 0, 0, 0, hipChannelFormatKindFloat);
        }

#if defined(__HIP_PLATFORM_HCC__)
        // SG!! set (wrong) hipArrayCubemap flag, since at the moment 3D texture can
        // be created only with this type of array.
        // Later it should be replaced by hipArraySurfaceLoadStore flag.
        HipArray3D vol_Array(arrayFormat, volSize.x, volSize.y, volSize.z, hipArrayCubemap);
#else
        HipArray3D vol_Array(arrayFormat, volSize.x, volSize.y, volSize.z, hipArraySurfaceLoadStore);
#endif
        HipTextureObject3D texObj(&vol_Array);
        ...
        if (mpi_part == 0) 
        {
            ...
            for (int i = 0; i < projCount; i++) 
            {
                ...
                log << indexList[i];
                if (i < projCount - 1) log << ", ";
            }
            log << endl;
            printf("\b \n\n");
        }

        cout << "construction of reconstructor.." << endl;
        Reconstructor reconstructor(aConfig, proj, projSource, markers, *defocus, modules, mpi_part, mpi_size, vol_Array.GetArray());
        cout << "construction of reconstructor done" << endl;

        if (mpi_part == 0) printf("Free Memory on device after allocations: %lu MB\n", hipCtx->GetFreeMemorySize() / 1024 / 1024);
        /////////////////////////////////////
        /// Filter Projections
        /////////////////////////////////////
        if (mpi_part == 0) 
        {
            ...
            if (!skipFilter && !reconstructor.ComputeFourFilter()) 
            {
                log << SimpleLogger::LOG_ERROR;
            ...
            for (int i = 0; i < projSource->GetProjectionCount(); i++) 
            {
                ...
                // projSource->GetProjection(i) always points to an array with an
                // element size of 4 bytes, Even if original data is stored in shorts!
                // We can therfor cast data and keep the same pointer.
        ...
        if (aConfig.WBP_NoSART) SIRTcount = 1;
        if (aConfig.WBP_NoSART) aConfig.Iterations = 1;
        ...
        /* AS Sergey Measurement variables ?
        static int istat = 0;
        string stat;
        ...
        */
        ...
        for (int iter = 0; iter < aConfig.Iterations; iter++) 
        {
            for (uint SIRTstep = 0; SIRTstep < (uint)(projCount + SIRTcount - 1) / (uint)SIRTcount; SIRTstep++) 
            {
                if (!aConfig.WBP_NoSART) // normal SART/SIRT reconstruction
                {
                    ...
                    {
                        if (SIRTstep * SIRTcount + i >= (uint)projCount)
                            continue;
                        int index = indexList[SIRTstep * (uint)SIRTcount + i];
                        // AS SG stat = string("test/") + to_string(istat++);
                        ...
                        if (aConfig.FP16Volume) 
                        {
                            ...
                            // emwrite("testDist.em", test, proj.GetWidth(),
                            // proj.GetHeight()); reconstructor.CopyProjectionToHost(test);
                            // emwrite("testProj.em", test, proj.GetWidth(),
                            // proj.GetHeight());/**/
                            if (mpi_part == 0)
                                reconstructor.Compare(volFP16, projSource->GetProjection(index), index);
                            // reconstructor.CopyProjectionToHost(test);
                            // emwrite("testComp.em", test, proj.GetWidth(),
                            // proj.GetHeight());/**/
                        } 
                        else 
                        {
                            // AS SG clock_t fpStart = clock();
                            reconstructor.ForwardProjection(vol, texObj, index, volumeIsEmpty);
                            // AS SG fpTime += (double)(clock() - fpStart);
                            if (mpi_part == 0) 
                            {
                                // AS SG float *test = new float[proj.GetWidth() * proj.GetHeight()];
                                // reconstructor.CopyDistanceImageToHost(test);
                                // emwrite("test/testDist.em", test, proj.GetWidth(),
                                // proj.GetHeight()); textwrite(
                                // (stat+string("testDist.txt")).c_str(), test, proj.GetWidth(),
                                // proj.GetHeight()); reconstructor.CopyProjectionToHost(test);
                                // emwrite("test/testProj.em", test, proj.GetWidth(),
                                // proj.GetHeight()); textwrite(
                                // (stat+string("testProj.txt")).c_str(), test, proj.GetWidth(),
                                // proj.GetHeight());
                                /* AS SG 
                                if (mpi_part == 0) {
                                printf(
                                ...
                                printf("Iter. %3i on proj. %3i/%3i, index %3i. CP  ",
                                        iter + 1, SIRTstep * (uint)SIRTcount + i + 1,
                                        projCount, index);
                                fflush(stdout);
                                }
                                clock_t compStart = clock();
                                */
                                reconstructor.Compare(vol, projSource->GetProjection(index), index);
                                // AS SG compTime += (double)(clock() - compStart);
                                // reconstructor.CopyProjectionToHost(test);
                                // emwrite("test/testComp.em", test, proj.GetWidth(),
                                // proj.GetHeight()); textwrite(
                                // (stat+string("testComp.txt")).c_str(), test, proj.GetWidth(),
                                // proj.GetHeight());
                                // AS SG delete[] test;
                                // exit(0);
                            }
                ...
                for (uint i = 0; i < (uint)SIRTcount; i++) 
                {
                    if (SIRTstep * (uint)SIRTcount + i >= (uint)projCount) 
                        continue;
                    // AS SG stat = string("test/") + to_string(istat++);
                    ...
                    {
                        // AS SG clock_t bpStart = clock();
                        reconstructor.BackProjection(vol, index, (float)SIRTcount);
                        // AS SG bpTime += (double)(clock() - bpStart);
                    }
                    ...
                    float *test = new float[proj.GetWidth() * proj.GetHeight()];
                    reconstructor.CopyProjectionToHost(test);
                    textwrite((stat + string("testBP.txt")).c_str(), test,
        ...
        /* AS SG 
        fpTime = fpTime / CLOCKS_PER_SEC;
        compTime = compTime / CLOCKS_PER_SEC;
        bpTime = bpTime / CLOCKS_PER_SEC;
        */

        if (mpi_part == 0)
        {
            printf("\n\nTotal time for reconstruction: %.2i:%.2i min.", (int)floor(runtime / 60.0), (int)floor(((runtime / 60.0) - floor(runtime / 60.0)) * 60.0));
            /* AS SG 
            printf("\n\nForward projection time: %.2i:%.2i min.", (int)floor(fpTime / 60.0), (int)floor(((fpTime / 60.0) - floor(fpTime / 60.0)) * 60.0));
            printf("\n\nComparison time: %.2i:%.2i min.", (int)floor(compTime / 60.0), (int)floor(((compTime / 60.0) - floor(compTime / 60.0)) * 60.0));
            printf("\n\nBack projection time: %.2i:%.2i min.", (int)floor(bpTime / 60.0), (int)floor(((bpTime / 60.0) - floor(bpTime / 60.0)) * 60.0));
            */
            printf("\n\n");
        }
        ...
        if (mpi_part == 0) 
        {
            ...
            else 
            {
                ...
                if (aConfig.FP16Volume && !aConfig.WriteVolumeAsFP16) 
                {
                    ...
                    float *volTemp_h =
                    new float[aConfig.RecDimensions.x * aConfig.RecDimensions.y];
                    for (int zSlice = 0; zSlice < dim.z; zSlice++) 
                    {
                        reconstructor.ConvertVolumeFP16(volFP16, volTemp_h, zSlice);
                        mVol->write((char *)volTemp_h, dimI);
                        mVol->flush();
                        printf("\b\b\b\b% 3d%%", (zSlice + 1) * 100 / (int)volFP16->GetDimension().z);
                        fflush(stdout);
                    }
                    mVol->close();
                    fflush(stdout);
...
#ifdef USE_MPI
                int ack = 0;
                if (mpi_size > 1) MPI_Send(&ack, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);
#endif
            }
        }
#ifdef USE_MPI
        ...
        {
            int ack = 0;
            MPI_Recv(&ack, 1, MPI_INT, mpi_part - 1, 0, MPI_COMM_WORLD,
                    MPI_STATUS_IGNORE);
            ...
            if (aConfig.FP16Volume && !aConfig.WriteVolumeAsFP16) 
            {
                ...
                else 
                {
                    ...
                    for (int zSlice = 0; zSlice < dim.z; zSlice++) 
                    {
                        ...
                        mVol->write((char *)volTemp_h, dimI);
                        mVol->flush();
                        printf("\b\b\b\b% 3d%%", (alreadyDone + zSlice + 1) * 100 / (int)volFP16->GetDimension().z);
                        fflush(stdout);
                    }
                    mVol->close();
                    fflush(stdout);
    ...
    HipContext::DestroyContext(hipCtx);
#ifdef USE_MPI
    MPI_Finalize();
#endif
}
